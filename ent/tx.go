...
func (tx *Tx) init() {
	tx.Bjd = NewBjdClient(tx.config)
	tx.Demol = NewDemolClient(tx.config)
	tx.Jibun = NewJibunClient(tx.config)
	tx.Perms = NewPermsClient(tx.config)
}
...
// applies a query, for example: Bjd.QueryXXX(), the query will be executed
